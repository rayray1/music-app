...
function App() {
  ...
  return (
    <div className="flex">
      <Sidebar playList={data} handleClickSidebar={handleClickSidebar} />
      {data && data.length > 0 && (
        <div className="container mx-auto mt-12 flex flex-col space-y-12">
          <SongCard selectedSong={data[songIndex]} />
          <PlayBar />
          <PlayBarControls
            playing={playing}
            togglePlayPause={togglePlayPause}
            handleClickNextSong={handleClickNextSong}
            handleClickPreviousSong={handleClickPreviousSong}
          />
        </div>
      )}
    </div>
